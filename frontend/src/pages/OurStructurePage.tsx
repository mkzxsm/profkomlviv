...
const StructurePage: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [facultyRes, departmentRes, teamRes] = await Promise.all([
          axios.get<Faculty[]>(
            `${import.meta.env.VITE_API_URL}/api/faculties`,
            {
  ...
  const getCountWord = (count: number): string => {
    ...
    const currentWords =
      selectedType === FACULTY_TYPE ? words.faculty : words.department;
            ...
            <div className="relative w-full md:w-auto">
              ...
              >
                <span className="truncate pr-2 text-sm md:text-base">
                  {
                    structureTypes.find((type) => type.id === selectedType)
                      ?.label
                  }
                </span>
                <ChevronDown
