...
const StructureManager: React.FC<StructureManagerProps> = ({
  ...
  const [modalType, setModalType] = useState<"faculty" | "department">(
    "faculty",
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      const isFaculty = selectedType === FACULTY_TYPE;
      const endpoint = isFaculty ? "faculties" : "departments";
      const item = (isFaculty ? facultyData : departmentData).find(
        (f) => f.id === id,
      );
  ...
  const currentData = isFacultyView
    ? enrichedFacultyData
    : enrichedDepartmentData;
  ...
  const filteredData = useMemo(() => {
    ...
    return currentData.filter((item) => {
      const extra = isFacultyView
        ? (item as Faculty).summary
        : (item as Department).description;
      return (
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={
              isFacultyView
                ? "Пошук профбюро за назвою..."
                : "Пошук відділу за назвою..."
            }
            value={searchTerm}
